 ...
 *
 *      $Id: class/template.php 2012-06-13 17:11:00 Beijing tengattack $
 */
...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		$basefile = basename(EI_ROOT.$tplfile, '.htm');
		$file == 'common/header' && defined('CURMODULE') && CURMODULE && $file = 'common/header_'.CURMODULE;
		$this->file = $file;

		if($fp = @fopen(EI_ROOT.$tplfile, 'r')) {
			$template = @fread($fp, filesize(EI_ROOT.$tplfile));
			fclose($fp);
		} elseif($fp = @fopen($filename = substr(EI_ROOT.$tplfile, 0, -4).'.php', 'r')) {
		...
		$template = "<? if(!defined('IN_EI')) exit('Access Denied'); {$headeradd}?>\n$template";
		...
		if(!@$fp = fopen(EI_ROOT.$cachefile, 'w')) {
			$this->error('directory_notfound', dirname(EI_ROOT.$cachefile));
		}
		...
		//$this->writetocsscache();	//css
		//$this->writetojscache();	//js
	}
	
	function writetojscache() {
		$dir = EI_ROOT.'static/js/';
		$dh = opendir($dir);
		$remove = array(
		array(
			'/(^|\r|\n)\/\*.+?\*\/(\r|\n)/is',
		...
		while(($entry = readdir($dh)) !== false) {
			if(fileext($entry) == 'js') {
				...
				if(@$fp = fopen(EI_ROOT.'./data/cache/'.$entry, 'w')) {
					fwrite($fp, $jsdata);
					fclose($fp);
				} else {
	...
	function writetocsscache() {
		global $_G;
		$dir = EI_ROOT.'./template/default/common/';
		$dh = opendir($dir);
		$data['staticurl'] = STATICURL;
		
		while(($entry = readdir($dh)) !== false) {
			if(fileext($entry) == 'css') {
				$cssfile = EI_ROOT.'./'.$data['tpldir'].'/common/'.$entry;
				!file_exists($cssfile) && $cssfile = $dir.$entry;
				$cssdata = @implode('', file($cssfile));
				if(file_exists($cssfile = EI_ROOT.'./'.$data['tpldir'].'/common/extend_'.$entry)) {
					$cssdata .= @implode('', file($cssfile));
				}
				if(is_array($_G['setting']['plugins']['available']) && $_G['setting']['plugins']['available']) {
					foreach($_G['setting']['plugins']['available'] as $plugin) {
						if(file_exists($cssfile = EI_ROOT.'./src/plugin/'.$plugin.'/template/extend_'.$entry)) {
							$cssdata .= @implode('', file($cssfile));
						}
					}
				}
				...
				if(@$fp = fopen(EI_ROOT.'./data/cache/style_'.$entry, 'w')) {
					fwrite($fp, $cssdata);
					fclose($fp);
				} else {
					exit('Can not write to cache files, please check directory ./data/ and ./data/cache/ .');
	...
	function languagevar($var) {
		...
		if(!isset($langvar[$var])) {
			$lang = array();
			@include EI_ROOT.'./src/lang/'.$langname.'/template.php';
			...
			if(!$isplugin) {
				...
				@include EI_ROOT.'./src/lang/'.$langname.'/'.$path.'/template.php';
				...
				if(empty($_G['config']['plugindeveloper'])) {
					...
					if(@include(EI_ROOT.'./data/plugindata/'.$vars[0].'.lang.php')) {
	...
	function loadcsstemplate() {
		global $_G;
		$scriptcss = '<link rel="stylesheet" type="text/css" href="data/cache/style_common.css?{VERHASH}" />';
		$content = $this->csscurmodules = '';
		$content = @implode('', file(EI_ROOT.'./data/cache/style_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen(EI_ROOT.'./data/cache/style_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
				fwrite($fp, $this->csscurmodules);
				fclose($fp);
			} else {
				exit('Can not write to cache files, please check directory ./data/ and ./data/cache/ .');
			}
			$scriptcss .= '<link rel="stylesheet" type="text/css" href="data/cache/style_'.$_G['basescript'].'_'.CURMODULE.'.css?{VERHASH}" />';
		}
	...
	function error($message, $tplname) {
		ei_error::template_error($message, $tplname);
	}
