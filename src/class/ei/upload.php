 ...
 *
 *      $Id: class/ei/upload.php 2012-06-18 07:45:00 Beijing tengattack $
 */
...
Class ei_upload{
	...
	function save($ignore = 0, $uid = 0) {
		...
		$this->attach['target'] = EI_ROOT.'./data/attach/'.$this->attach['attachment'];
	...
	function get_image_info($target, $allowswf = false) {
		$ext = ei_upload::fileext($target);
		$isimage = ei_upload::is_image_ext($ext);
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		$check_exists && ei_upload::check_dir_exists($type, $subdir1, $subdir2);
	...
	function check_dir_exists($type = '', $sub1 = '', $sub2 = '') {
		...
		$type = ei_upload::check_dir_type($type);

		$basedir = (EI_ROOT.'./data/attach');
		...
		if(!$res) {
			$res = $typedir && ei_upload::make_dir($typedir);
			$res && $subdir1 && ($res = ei_upload::make_dir($subdir1));
			$res && $subdir1 && $subdir2 && ($res = ei_upload::make_dir($subdir2));
		}
	...
	function save_to_local($source, $target) {
		if(!ei_upload::is_upload_file($source)) {
			$succeed = false;
