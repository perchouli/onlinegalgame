 ...
 *
 *      $Id: class/ei/application.php 2012-06-13 11:04:00 Beijing tengattack $
 */
...
class ei_application extends ei_base{
	...
	private function _init_env() {
		...
		if(!defined('EI_CORE_FUNCTION') && !@include(EI_ROOT.'./src/func/core.php')) {
	...
	private function _init_config() {
		...
		$_config = array();
		@include EI_ROOT.'./src/conf/global.php';
		if(empty($_config)) {
			if(!file_exists(EI_ROOT.'./data/install.lock')) {
	...
	private function _init_session() {
		...
		$this->session = $sessionclose ? new ei_session_close() : new ei_session();
	...
	private function _init_user() {
		if($this->init_user) {
			...
			list($ei_pw, $ei_uid) = empty($auth) || count($auth) < 2 ? array('', '') : $auth;

			if($ei_uid) {
				$user = getuserbyuid($ei_uid, 1);
			}

			if(!empty($user) && $user['password'] == $ei_pw) {
				if(isset($user['_inarchive'])) {
					C::t('common_member_archive')->move_to_master($ei_uid);
				}
	...
	private function _init_setting() {
		...
		require_once EI_ROOT.'./src/inc/settings.php';
	}
