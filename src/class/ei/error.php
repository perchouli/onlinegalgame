 ...
 *
 *      $Id: class/ei/error.php 2012-06-13 11:37:00 Beijing tengattack $
 */
...
class ei_error
{
	...
	public static function system_error($message, $show = true, $save = true, $halt = true) {
		...
		list($showtrace, $logtrace) = ei_error::debug_backtrace();

		if($save) {
			$messagesave = '<b>'.$message.'</b><br><b>PHP:</b>'.$logtrace;
			ei_error::write_error_log($messagesave);
		}

		if($show) {
			if(!defined('IN_MOBILE')) {
				ei_error::show_error('system', "<li>$message</li>", $showtrace, 0);
			} else {
				ei_error::mobile_show_error('system', "<li>$message</li>", $showtrace, 0);
			}
		}
	...
	public static function template_error($message, $tplname) {
		$message = lang('error', $message);
		$tplname = str_replace(EI_ROOT, '', $tplname);
		$message = $message.': '.$tplname;
		ei_error::system_error($message);
	}

	public static function debug_backtrace() {
		$skipfunc[] = 'ei_error->debug_backtrace';
		$skipfunc[] = 'ei_error->db_error';
		$skipfunc[] = 'ei_error->template_error';
		$skipfunc[] = 'ei_error->system_error';
		...
		foreach ($debug_backtrace as $k => $error) {
			$file = str_replace(EI_ROOT, '', $error['file']);
	...
	public static function db_error($message, $sql) {
		...
		ei_error::show_error('db', $msg, $showtrace, false);
		unset($msg, $phperror);
		...
		ei_error::write_error_log($errormsg);
		exit();
	...
	public static function exception_error($exception) {
		...
		foreach ($trace as $error) {
				...
				if(!empty($error['args'])) {
					...
					foreach($error['args'] as $arg) {
						...
						if(is_array($arg)) {
							...
							$fun .= (defined('EI_DEBUG') && EI_DEBUG) ? $arg : '%d';
						} elseif(is_float($arg)) {
							$fun .= (defined('EI_DEBUG') && EI_DEBUG) ? $arg : '%f';
						} else {
							$fun .= (defined('EI_DEBUG') && EI_DEBUG) ? '\''.dhtmlspecialchars(substr(self::clear($arg), 0, 10)).(strlen($arg) > 10 ? ' ...' : '').'\'' : '%s';
						}
						$mark = ', ';
					}
				}
			...
			$phpmsg[] = array(
			    'file' => str_replace(array(EI_ROOT, '\\'), array('', '/'), $error['file']),
			    'line' => $error['line'],
...
<body>
<div id="container">
<h1>EarlyImbrian $title Error</h1>
...
<tr>
	<td id="bodytitle" width="100%">EarlyImbrian $title Error </td>
</tr>
EOT;
...
EOT;
	...
	public static function write_error_log($message) {
		...
		$message = ei_error::clear($message);
		$time = time();
		$file =  EI_ROOT.'./data/log/'.date("Ym").'_errorlog.php';
		$hash = md5($message);
		...
		$uri = 'Request: '.dhtmlspecialchars(ei_error::clear($_SERVER['REQUEST_URI']));
