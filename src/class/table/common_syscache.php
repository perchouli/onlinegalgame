 ...
 *
 *      $Id: class/table/common_syscache.php 2012-06-13 12:49:00 Beijing tengattack $
 */
...
class table_common_syscache extends ei_table
{
	...
	public function fetch_all($cachenames) {
		...
		if($this->_isfilecache) {
			...
			foreach($cachenames as $cachename) {
				if(!@include_once(EI_ROOT.'./data/cache/cache_'.$cachename.'.php')) {
		...
		while($syscache = DB::fetch($query)) {
			...
			if($this->_isfilecache) {
				...
				if(($fp = @fopen(EI_ROOT.'./data/cache/cache_'.$syscache['cname'].'.php', 'wb'))) {
					fwrite($fp, "<?php\n//EarlyImbrian cache file, DO NOT modify me!\n//Identify: ".md5($syscache['cname'].$cachedata.getglobal('config/security/authkey'))."\n\n$cachedata?>");
					fclose($fp);
				}
			}
		}
	...
	public function insert($cachename, $data) {
		...
		$this->_isfilecache && @unlink(EI_ROOT.'./data/cache/cache_'.$cachename.'.php');
	}
