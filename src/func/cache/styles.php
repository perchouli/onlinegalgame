 ...
 *
 *      $Id: function/cache/styles.php 2012-06-13 19:14:00 Beijing tengattack $
 */
	...
	foreach(C::t('common_style')->fetch_all_data(true) as $data) {
		...
		if($data['extstyle']) {
			...
			foreach($extstyle as $dir) {
				if(file_exists($extstylefile = EI_ROOT.$data['tpldir'].'/style/'.$dir.'/style.css')) {
		...
		if(fileext($entry) == 'css') {
			...
			if(is_array($_G['setting']['plugins']['available']) && $_G['setting']['plugins']['available']) {
				foreach($_G['setting']['plugins']['available'] as $plugin) {
					if(file_exists($cssfile = EI_ROOT.'./source/plugin/'.$plugin.'/template/extend_'.$entry)) {
						$cssdata .= @implode('', file($cssfile));
					}
				}
			}
			...
			if(@$fp = fopen(EI_ROOT.'./data/cache/style_'.$data['styleid'].'_'.$entry, 'w')) {
				fwrite($fp, $cssdata);
				fclose($fp);
			} else {
